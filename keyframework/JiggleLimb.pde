...
public class JiggleLimb {
  ...
  public void update(float ox, float oy, float ex, float ey, float a) {
    ...
    for (int x = 1; x < particles.length; x++) {
      float tvx = map(x, 0, particles.length, tops[0].x, tops[1].x);
      float tvy = map(x, 0, particles.length, tops[0].y, tops[1].y);
      float bvx = map(x, 0, particles.length, bots[0].x, bots[1].x);
      float bvy = map(x, 0, particles.length, bots[0].y, bots[1].y);
